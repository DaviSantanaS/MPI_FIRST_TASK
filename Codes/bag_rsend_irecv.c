...
int main(int argc, char *argv[]) {
    ...
    /* Barreira estratégica para tentar garantir que todos entrem na zona de comunicação juntos */
    ...
    if (meu_ranque == 0) { 
        ...
        for (dest = 1; dest < num_procs; dest++) {
            if (inicio <= n) {
                MPI_Rsend(&inicio, 1, MPI_INT, dest, TAG_TRABALHO, MPI_COMM_WORLD);
                trabalhadores_ativos++;
                inicio += TAMANHO;
            } else {
                MPI_Rsend(&inicio, 1, MPI_INT, dest, TAG_FIM, MPI_COMM_WORLD);
            }
        }
        ...
        while (1) {
            ...
            MPI_Rsend(&cont, 1, MPI_INT, raiz, TAG_TRABALHO, MPI_COMM_WORLD);
        }
    }
