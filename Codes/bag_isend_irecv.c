...
int main(int argc, char *argv[]) {
    ...
    MPI_Status estado;
    MPI_Request req_send, req_recv; /* Requisições independentes para envio e recebimento */
    ...
    if (meu_ranque == 0) { 
        ...
        for (dest = 1; dest < num_procs; dest++) {
            if (inicio <= n) {
                MPI_Isend(&inicio, 1, MPI_INT, dest, TAG_TRABALHO, MPI_COMM_WORLD, &req_send);
                MPI_Wait(&req_send, MPI_STATUS_IGNORE);
                trabalhadores_ativos++;
                inicio += TAMANHO;
            } else {
                MPI_Isend(&inicio, 1, MPI_INT, dest, TAG_FIM, MPI_COMM_WORLD, &req_send);
                MPI_Wait(&req_send, MPI_STATUS_IGNORE);
            }
        }
        ...
        while (1) {
            MPI_Irecv(&inicio, 1, MPI_INT, raiz, MPI_ANY_TAG, MPI_COMM_WORLD, &req_recv);
            MPI_Wait(&req_recv, &estado);
            ...
            MPI_Isend(&cont, 1, MPI_INT, raiz, TAG_TRABALHO, MPI_COMM_WORLD, &req_send);
            MPI_Wait(&req_send, MPI_STATUS_IGNORE);
        }
    }
